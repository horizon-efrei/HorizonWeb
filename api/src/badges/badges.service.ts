...
export class BadgesService {
	...
	public async createMany(createBadgesDto: CreateBadgesDto): Promise<Badge[]> {
		...
		for (const item of createBadgesDto.badges) {
			...
			if (sameSlugBadge) throw new BadRequestException(`Badge with same slug already exists : ${item.slug}`);

			const sameNameBadge = await this.badgeRepository.findOne({ name: item.name });
			if (sameNameBadge) throw new BadRequestException(`Badge with same name already exists : ${item.name}`);
		...
		try {
			...
			if (error instanceof UniqueConstraintViolationException) throw new BadRequestException('Error'); // 'Tag name already exists'
			throw error;
		}
