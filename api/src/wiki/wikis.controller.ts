...
export class WikisController {
  ...
  public async findAll(@Query() query: PaginateDto, @Param('category') category: string): Promise<PaginatedResult<Wiki>> {
    if (query.page)
...
/*   @Get(':category')
  //@CheckPolicies(ability => ability.can(Action.Read, Wiki))
  public async findAllCategory(@Param('category') category: string): Promise<Wiki | null> {
    return await this.wikisService.findAllCategory(category);
  } */
