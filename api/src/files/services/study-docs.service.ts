import {
 Body, ForbiddenException, Injectable, NotFoundException,
} from '@nestjs/common';
...
export class StudyDocsService {
  ...
  public async create(currentUser: User, @Body() body: CreateStudyDocDto,
    fileDocument: FileUpload): Promise<StudyDoc> {
    const upload = await this.studyDocModel.create({
      file: fileDocument,
      ...(body),
    });
