<template>
<footer>
      <div class="px-6 pt-6 pb-2 border-t-2 border-footer">
        <div class="flex flex-wrap justify-center">
          <div class="w-full md:w-1/4 text-center ">
            <h5 class="uppercase md:mb-3 font-bold text-0">
                Liens
            </h5>
            <ul class="mb-4">
              <li class="mt-2">
                <a
                  href="#"
                  class="hover:underline text-3 hover:text-orange-500"
                  >FAQ</a
                >
              </li>
              <li class="mt-2">
                <a
                  href="#"
                  class="hover:underline text-3 hover:text-orange-500"
                  >Aide</a
                >
              </li>
              <li class="mt-2">
                <a
                  href="#"
                  class="hover:underline text-3 hover:text-orange-500"
                  >Support</a
                >
              </li>
            </ul>
          </div>
          <div class="w-full md:w-1/4 text-center text-0">
            <h5 class="uppercase mt-2 md:mt-0 md:mb-3 font-bold">
              Légalité
            </h5>
            <ul class="mb-4">
              <li class="mt-2">
                <a
                  href="#"
                  class="hover:underline text-3 hover:text-orange-500"
                  >CGU</a
                >
              </li>
              <li class="mt-2">
                <a
                  href="#"
                  class="hover:underline text-3 hover:text-orange-500"
                  >RGPD</a
                >
              </li>
            </ul>
          </div>
          <div class="w-full md:w-1/4 text-center text-0">
            <h5 class="uppercase mt-2 md:mt-0 md:mb-3 font-bold">Réseaux</h5>
            <ul class="mb-4">
              <li class="mt-2">
                <a
                  href="https://www.instagram.com/horizon.efrei/"
                  class="hover:underline text-3 hover:text-orange-500"
                  >Instagram</a
                >
              </li>
              <li class="mt-2">
                <a
                  href="https://discord.gg/q22FW5xhvP"
                  class="hover:underline text-3 hover:text-orange-500"
                  >Discord</a
                >
              </li>
            </ul>
          </div>
          <div class="w-full md:w-1/4 text-center text-0">
            <h5 class="uppercase mt-2 md:mt-0 md:mb-3 font-bold">Horizon</h5>
            <ul class="mb-4">
              <li class="mt-2">
                <a
                  href="#"
                  class="hover:underline text-3 hover:text-orange-500"
                  >Blog</a
                >
              </li>
              <li class="mt-2">
                <a
                  href="#"
                  class="hover:underline text-3 hover:text-orange-500"
                  >Notre équipe</a
                >
              </li>
              <li class="mt-2">
                <a
                  href="#"
                  class="hover:underline text-3 hover:text-orange-500"
                  >Contact</a
                >
              </li>
            </ul>
          </div>
        </div>
      </div>
    </footer>
</template>

<script lang="js">
import { defineComponent } from 'vue'

export default defineComponent({
  name: 'Footer',
  props: {
  },
  methods: {
  }
})
</script>

<style scoped>
</style>
